...
class S3StorageAdapter implements StorageAdapterInterface
{
    ...
    public function __construct(string $disk = 's3', string $basePath = '', string $bucket = null)
    {
        $this->disk = $disk;
        $this->basePath = $basePath;
        $this->bucket = $bucket ?? config('filesystems.disks.s3.bucket');
    }
    ...
    public function url(string $identifier, array $options = []): ?string
    {
        ...
        if (isset($options['expires'])) {
            return Storage::disk($this->disk)->temporaryUrl($identifier, $options['expires']);
        }
