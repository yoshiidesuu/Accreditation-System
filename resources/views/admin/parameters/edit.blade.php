            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title mb-0">
                        <i class="fas fa-edit me-2"></i>Edit Parameter: {{ $parameter->title }}
                    </h3>
                    <div class="card-tools">
                        <a href="{{ route('admin.parameters.show', $parameter) }}" class="btn btn-info btn-sm me-2">
                ...
                <form action="{{ route('admin.parameters.update', $parameter) }}" method="POST" id="parameterForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <!-- Current Status Info -->
                        <div class="alert alert-info mb-4">
                            <div class="row">
                                <div class="col-md-3">
                                    ...
                                    <span class="badge bg-info">{{ $parameter->area->code }}</span>
                                ...
                                <div class="col-md-3">
                                    ...
                                    {{ $parameter->created_at->format('M d, Y') }}
                                </div>
                            </div>
                        </div>
                        ...
                        @if($parameter->hasContent())
                            <div class="alert alert-warning mb-4">
                                ...
                                Changing the field type may affect existing data compatibility.
                            </div>
                        @endif
                                    ...
                                    <div class="card-body">
                                                    ...
                                                    <select class="form-select @error('area_id') is-invalid @enderror" 
                                                        ...
                                                        @foreach($areas as $area)
                                                            <option value="{{ $area->id }}" 
                                                                {{ (old('area_id', $parameter->area_id) == $area->id) ? 'selected' : '' }}>
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                                      ...
                                                      placeholder="Optional description for this parameter">{{ old('description', $parameter->description) }}</textarea>
                                            @error('description')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox" 
                                                               ...
                                                               {{ old('required', $parameter->required) ? 'checked' : '' }}>
                                            ...
                                            <div class="col-md-3">
                                                <div class="mb-3">
                                                    ...
                                                    <input type="number" class="form-control @error('order') is-invalid @enderror" 
                                                           id="order" name="order" value="{{ old('order', $parameter->order) }}" 
                                                           min="0" step="1">
                                                    @error('order')
                                                        <div class="invalid-feedback">{{ $message }}</div>
                                                    @enderror
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox" 
                                                               ...
                                                               {{ old('active', $parameter->active) ? 'checked' : '' }}>
                                                        <label class="form-check-label" for="active">
                                                            Parameter is active
                                                        </label>
                                                    </div>
                                                    <div class="form-text">Inactive parameters won't be displayed in forms</div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('admin.parameters.show', $parameter) }}" class="btn btn-secondary">
...
<script>
let optionCounter = 0;
const existingOptions = @json(old('options', $parameter->options ?? []));
const existingValidationRules = @json(old('validation_rules', $parameter->validation_rules ?? []));
...
['title', 'description', 'required', 'order', 'active'].forEach(field => {
...
function updateValidationRules(type) {
    ...
    commonRules.forEach(rule => {
        if (rule.applicable.includes(type)) {
            ...
            div.className = 'mb-2';
            const existingValue = existingValidationRules[rule.name] || '';
            div.innerHTML = `
...
function addOption(label = '', value = '') {
    ...
    div.innerHTML = `
        <input type="text" class="form-control" name="options[${optionCounter}][label]" 
               value="${label}" placeholder="Option label" onchange="updatePreview()">
        <input type="text" class="form-control" name="options[${optionCounter}][value]" 
               value="${value}" placeholder="Option value" onchange="updatePreview()">
...
function updatePreview() {
    ...
    let html = `
        <div class="mb-3 ${!active ? 'opacity-50' : ''}">
            <label class="form-label">
                ${title}
                ${required ? '<span class="text-danger">*</span>' : ''}
                ${!active ? '<span class="badge bg-warning ms-2">Inactive</span>' : ''}
            </label>
    `;
